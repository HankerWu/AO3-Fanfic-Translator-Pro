...
import { Upload, FileText, Link as LinkIcon, AlertCircle, ChevronDown, ChevronUp, Sliders, X, Sparkles, Languages, ArrowRight, Loader2 } from 'lucide-react';
  ...
  const handleAutoGlossary = async () => {
     ...
     try {
         ...
         if (result) {
             ...
             // If empty string returned, it likely failed/caught error in service
      ...
      <div className="bg-white dark:bg-[#1a1a1a] rounded-3xl shadow-2xl shadow-gray-200/50 dark:shadow-black/50 border border-gray-100 dark:border-gray-800 overflow-hidden ring-1 ring-gray-900/5 transition-colors">
        ...
        <div className="p-8 space-y-8">
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <button onClick={props.onRestorePrompt} className="text-[10px] text-[#990000] hover:underline font-bold">{t.restorePrompt}</button>
                    </div>
          ...
          <button 
            ...
            className={`h-16 w-full rounded-2xl font-black uppercase tracking-widest text-lg shadow-xl shadow-red-900/10 flex items-center justify-center gap-3 transition-all transform active:scale-[0.98] ${props.isProcessing ? 'bg-gray-300 dark:bg-gray-700 cursor-not-allowed text-white' : 'bg-gradient-to-r from-[#990000] to-[#cc0000] text-white hover:shadow-red-900/20'}`}
          >
            {props.isProcessing ? <Loader2 className="animate-spin w-6 h-6"/> : <ArrowRight className="w-6 h-6"/>}
            {props.isProcessing ? t.translating : t.start}
          </button>
        </div>
      </div>
    </div>
